...
add_action('wp_enqueue_scripts', function () {
    ...
    // CSS generated by JS imports, like GLightbox CSS
    $build_js_css_fs = get_stylesheet_directory() . '/build/js/main.min.css';

    if ( file_exists( $build_js_css_fs ) ) {
        wp_enqueue_style(
            'theme-main-js-css',
            get_stylesheet_directory_uri() . '/build/js/main.min.css',
            ['theme-style'],
            theme_file_ver( $build_js_css_fs )
        );
    }
...
/* ---------------------------------
 * Block: midwest-ceramic-coating/about-spotlight
 * - Register editor script handle (deps ensure wp.* exists)
 * - Register block from block.json ONCE (idempotent)
 * --------------------------------- */
add_action('init', function () {
    $block_dir_fs  = trailingslashit( get_stylesheet_directory() ) . 'blocks/about-spotlight';
    $block_json_fs = $block_dir_fs . '/block.json';
    if ( ! file_exists( $block_json_fs ) ) {
        if ( defined('WP_DEBUG') && WP_DEBUG ) {
            error_log('[blocks] block.json not found at ' . $block_json_fs);
    ...
    // Register the editor script handle referenced by block.json ("editorScript": "theme-about-spotlight-editor")
    $editor_fs = $block_dir_fs . '/editor.js';
    if ( file_exists( $editor_fs ) ) {
        wp_register_script(
            'theme-about-spotlight-editor',
            trailingslashit( get_stylesheet_directory_uri() ) . 'blocks/about-spotlight/editor.js',
            [ 'wp-blocks', 'wp-element', 'wp-i18n', 'wp-block-editor', 'wp-editor' ],
            theme_file_ver( $editor_fs ),
            true
        );
    ...
    // Avoid duplicate registration if parent theme/plugin already did it
    $registry = WP_Block_Type_Registry::get_instance();
    if ( $registry->is_registered( 'midwest-ceramic-coating/about-spotlight' ) ) {
        return;
    }
